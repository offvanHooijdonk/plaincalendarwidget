...
public class EventsRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    private WidgetModel widgetOptions;
    private Context ctx;
    ...
    public void onDataSetChanged() {
        ...
        App.getAppDatabase().widgetDao().getById(widgetId)
                .flatMap(wBean -> App.getAppDatabase().calendarDao().getCalendarsForWidget(widgetId)
                        .map(calendarBeans -> {
    ...
    private void displayEvents(List<EventModel> list) {
        eventList.clear();
...
// TODO make beauty
        ...
        String eventDateText = DateHelper.formatEventDateRange(ctx, event.getDateStart(), event.getDateEnd(), event.isAllDay(),
                ...
                widgetOptions != null ? widgetOptions.getShowEndDate() : WidgetModel.ShowEndDate.NEVER);
