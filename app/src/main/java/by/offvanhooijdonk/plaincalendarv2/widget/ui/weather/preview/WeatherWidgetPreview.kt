...
fun WeatherWidgetPreview(
    ...
    Card(
        ...
        Row(Modifier.padding(dimens().spacingM)) {
            TodayMain(
                modifier = Modifier
                    .weight(1f)
                    .fillMaxHeight(),
            ...
            Column(
                ...
                TodayAdditionalInfo()
                Spacer(Modifier.height(dimens().spacingL + dimens().spacingS))
...
private fun TodayMain(
    ...
    Column(
        modifier = modifier.background(Color(0xFFFEF0FE)),
        verticalArrangement = Arrangement.Center,
    ) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text(text = weather.temperature, fontSize = 42.sp)
            Icon(
...
private fun TodayAdditionalInfo(modifier: Modifier = Modifier) {
    Row(modifier = modifier, horizontalArrangement = Arrangement.spacedBy(dimens().spacingM)) {
        Column {
...
fun ForecastBadge(
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text(time, fontSize = 10.sp, lineHeight = 12.sp)
        ...
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(dimens().spacingXS)) {
            ...
            Text("$rainProbability%", fontSize = 10.sp, lineHeight = 12.sp)
        }
    }
}
